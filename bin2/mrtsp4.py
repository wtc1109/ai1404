    ...
    try:
        ...
        if 0 != _a1:
            ...
            for _file in _files:
                ...
                if 0 != Keep_moving_video or (3 != _video_type_int and 33 != _video_type_int and 333 != _video_type_int):
                    if _file[8] < 50:
                        try:
...
def video_files_log(_cur,_table):
    ...
    try:
        _sql = "select CarCount,parking1State,parking2State,parking3State from aiouttable where cameraID='%s' " \
               "and cmosID=%d" % (_table[0], _table[1])
        if 0 != _cur.execute(_sql):
            _aiState = _cur.fetchone()
            if None == _aiState:
                _aiState = (3, 3, 3, 3)
        else:
            _aiState = (3, 3, 3, 3)
        ...
        if 0 != _aiState[0]:
            _video_type = ""
            for i in range(_aiState[0]):
                if _table[6][i] == '4':
                    _video_type += '4'
                elif _table[6][i] == '1' and 0 == _aiState[1+i]:#out
                    _video_type += '2'
                elif _table[6][i] == '0' and 1 == _aiState[1+i]:#in
                    _video_type += '1'
                elif _table[6][i] == '0' and 0 == _aiState[1+i]:
                    _video_type += '3'
                elif _table[6][i] == '1' and 1 == _aiState[1+i]:
                    _video_type += '3'
                else:
        ...
        mylogger.error(str(e))
        mylogger.error(_sql)
        mylogger.error("start_aiout=%s,len=%d"%(_table[6],_aiState[0]))
    return
...
def start_video_record(_cur, _table):
    ...
    try:
        ...
        """
        _sql ="select CarCount,parking1State,parking2State,parking3State from aiouttable where cameraID='%s' and cmosID=%d" % (_table[0], _table[1])
        if 0 != _cur.execute(_sql):
            _aiState = _cur.fetchone()
        else:
            mylogger.warning("None = "+_sql)
            _aiState = (3,3,3,3)
        """
...
def check_max_rtsp_client_ch(_cur):
    ...
    if 0 != _a1:
        ...
        if 0 != _max_def and _max_def > 576:
            ...
            video_def_dictL = ((1920, 8), (1600, 16), (1280, 32), (1056, 48), (864, 128), (640, 128), (576, 128))
