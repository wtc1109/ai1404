...
class get_ai_out2_info(threading.Thread):
    ...
    def run(self):
        ...
        try:
            _sql_str = "select max(LatestUpdate) from aiouttable2"
            aiout2_cur.execute(_sql_str)
            _datetime, = aiout2_cur.fetchone()
            if None == _datetime:
                _datetime = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            _sql_str = "SELECT * FROM aiouttable2"
            if 0 != aiout2_cur.execute(_sql_str):
                _settings = aiout2_cur.fetchall()
                for _setting in _settings:
                    AI_out2s.update({_setting[0]: [info for info in _setting]})
        except Exception, e:
            self.__logger.error(str(e))
            self.__logger.error(_sql_str)
            if 'MySQL server has gone away' in str(e):
        ...
        while True:
            try:
                _sql_str = "select * from aiouttable2 where LatestUpdate >'%s'" % _datetime
                if 0 != aiout2_cur.execute(_sql_str):
                    _new_settings = aiout2_cur.fetchall()
                    for _new_set in _new_settings:
                        AI_out2s.update({_new_set[0]: [info for info in _new_set]})
                    _sql_str = "select max(LatestUpdate) from aiouttable2"
                    aiout2_cur.execute(_sql_str)
                    _datetime, = aiout2_cur.fetchone()
                else:
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                #_yuv_filename = str(_dic_info["pic_full_name"]).split("_")
                _file = "%d_%02d%02d%02d_%03d" % (_dic_info["plateGet"], localtime1.tm_hour, localtime1.tm_min,
                                                  localtime1.tm_sec, t2)
                _yuv_filename1 = _file+'_%dx%d.yuv'%(_width,_height)
                _new_yuv_name = os.path.join(_day_str, _yuv_filename1)
                shutil.copy(_dic_info["pic_full_name"], _new_yuv_name)
                if os.path.getsize(_new_yuv_name) != _height*_width*3:
                    self.__logger.info("%s file size is %s, while mq size is %d * %d= %d"
                                       %(str(_dic_info["pic_full_name"]),os.path.getsize(_new_yuv_name),
                                         _width,_height,_width*_height*3))
                    continue
                """
                ...
                self.__logger.info(_re_yuv_path)
                ...
                self.__logger.info(_jpg_tmp_name)
            ...
            try:
                ...
                if wdi_time30sec != _new_32sec:
                    ...
                    _sql_str = "update WatchdogTable set renewTimet=%d where pid=%d and ServerID='%s'" \
                               % (int(time.time()), os.getpid(), ServerID)
                    title_cur.execute(_sql_str)
                ...
                _sql_str = "insert into reouttable(cameraID,cmosID)values('%s',0)ON DUPLICATE KEY UPDATE Plate%dupdateTimet=0;" \
                           % (_dic_info["cam_id"], _dic_info["plateGet"])
                title_cur.execute(_sql_str)
                _sql_str = "insert into reoutfiltertable(cameraID,cmosID)values('%s',0)ON DUPLICATE KEY UPDATE Plate%dupdateTimet=0;" \
                           % (_dic_info["cam_id"], _dic_info["plateGet"])
                title_cur.execute(_sql_str)
                _sql_str = "insert into refiltertable(Spaceid)values('%s0%d')ON DUPLICATE KEY UPDATE cameraID='%s';" \
                           % (_dic_info["cam_id"], _dic_info["plateGet"], _dic_info["cam_id"])
                title_cur.execute(_sql_str)
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    #_O_which_slot += 1
                    self.__logger.info("get %s bay%d VLPR out staisitc:%s, %s"%(_cam_id, _O_which_slot, _staisitc_plate,_plate))
                ...
                if False == has_result:  # not result
                    ...
                    try:
                        _sql_str = "update reouttable set Plate%dupdateTimet=%d,Plate%dNumber='%c%c%s'," \
                                   "Plate%dtype='%s' where cameraID='%s'" \
                                   % (_O_which_slot, _timeSec, _O_which_slot, _plate[0], _plate[1], str(_plate[2:]),
                                      _O_which_slot,
                                      _plate_type, _cam_id)
                        result_cur.execute(_sql_str)
                ...
                try:
                    ...
                    for i in range(1, 9):
                        _sql_str += "PlateNumber%d='%s'," % (i, str(_O_use_by_statistic_plate[i - 1].strip('\0')))

                    _sql_str += "cmosID=0 where Spaceid='%s0%d'" % (_cam_id,  _O_which_slot)
                    ...
                    if _staisitc_plate != _plate and '' != _staisitc_plate and '1' == Backup_re_jpg:
                        ...
                        try:
                            ...
                            _day_file_yuv = "%d%02d%02d/%s0/%s.yuv" % (
                            localtime1.tm_year, localtime1.tm_mon, localtime1.tm_mday, _cam_id, _after_filename)
                    ...
                    if 0 == len(_find_file) or len(_find_file) > 1:
                        self.__logger.info("not found %s"%(_path + _filename))
                        continue
                    """
                    if _cam_id in AI_out2s:
                        _ai_out2 = AI_out2s[_cam_id]
                        if 3 != _ai_out2[4]:
                            _sql_str = "update reoutfiltertable set Plate%dNumber=null,Plate%dtype=null"%(_O_which_slot,_O_which_slot)
                            result_cur.execute(_sql_str)
                    """
                    _sql_str = ""
                    if '' == _staisitc_plate:
                        _sql_str = "UPDATE reoutfiltertable SET Plate%dupdateTimet=%d,Plate%dDatetime=now()," \
                                   "Plate%dNumber=null,Plate%dtype=null WHERE cameraID='%s'" \
                                   % (_O_which_slot, _timeSec, _O_which_slot, _O_which_slot
                                      , _O_which_slot, _cam_id)
                    else:
                        _sql_str = "UPDATE reoutfiltertable SET Plate%dupdateTimet=%d,Plate%dDatetime=now()," \
                                   "Plate%dNumber='%s',Plate%dtype='%s' WHERE cameraID='%s'" \
                                   % (_O_which_slot, _timeSec, _O_which_slot, _O_which_slot,
                                      _staisitc_plate, _O_which_slot, _plate_type, _cam_id)
                    result_cur.execute(_sql_str)

                    _sql_str = "select * from reoutfiltertable where cameraID='%s'"%_cam_id
                    result_cur.execute(_sql_str)
                    _reoutfilter = result_cur.fetchone()
                    _comment = "sn:%s; ip: %s; "%(_cam_id,_ip)
                    if _cam_id in AI_out2s:
                        _bays = AI_out2s[_cam_id][4]
                    else:
                        _bays = 3
                    for i in range(_bays):
                        if None != _reoutfilter[4+5*i]:
                            _comment += "plate%d:%s,plate%dtype:%s "%(i+1,_reoutfilter[4+5*i],i+1,_reoutfilter[5+5*i])


                    #_sql_str = "insert into jpgexif(file,comment)values('%s','%s')" % (_find_file[0], _comment)
                    #result_cur.execute(_sql_str)
                    self.__logger.info("exif %s with %s"%(_comment,_find_file[0]))
...
def msg_consumer(channel, method, header, body):
    global mylogger,  msg_props, interval_sec,REOUT_CH,msgsnd_ipc, msgrcv_ipc,AI_daybook_en,MSG_send_error_cnt, Backup_re_jpg
    ...
    try:
        ...
        try:
            ...
            #level_line1 = [_re_setting[i]*2304/100 for i in range(5, 13)]  # ; // 12
            level_line1 = [0]*12
            level_line1[4] = _ai_out2[25]*_width/8192
            level_line1[5] = _ai_out2[26] * _height / 8192
            #mylogger.info("level_line1")
            undis_param_t1 = struct.pack('4B8i', int(_re_setting[3]*10),_ai_out2[4],_re_setting[2],_re_setting[4], *[level_line1[i] for i in range(8)])
            ...
            #mylogger.info("level_line1")
            car_coord = [0]*12
            car_coord[0] = _ai_out2[38+4*(_dic_info["plateGet"]-1)]*_width/8192
            car_coord[1] = _ai_out2[39 + 4 * (_dic_info["plateGet"] - 1)] * _height / 8192
            car_coord[2] = _ai_out2[40 + 4 * (_dic_info["plateGet"] - 1)] * _width / 8192
            car_coord[3] = _ai_out2[41 + 4 * (_dic_info["plateGet"] - 1)] * _height / 8192
            plate_location = [0]*12
            _x = _ai_out2[50 + 8 * (_dic_info["plateGet"] - 1)] * _width / 8192
            if _x >= _dic_info["LTx"]:
                plate_location[0] = _x
            else:
                plate_location[0] = 0
            _y = _ai_out2[51 + 8 * (_dic_info["plateGet"] - 1)] * _height / 8192
            if _y >= _dic_info["LTy"]:
                plate_location[1] = _y
            else:
                plate_location[1] = 0
            _x = _ai_out2[52 + 8 * (_dic_info["plateGet"] - 1)] * _width / 8192
            if _x <= _dic_info["RBx"]:
                plate_location[2] = _x
            else:
                plate_location[2] = _dic_info["RBx"]
            _y = _ai_out2[53 + 8 * (_dic_info["plateGet"] - 1)] * _height / 8192
            if _y <= _dic_info["RBy"]:
                plate_location[3] = _y
            else:
                plate_location[3] = _dic_info["RBy"]

            _x = _ai_out2[54 + 8 * (_dic_info["plateGet"] - 1)] * _width / 8192
            if _x >= _dic_info["LTx"]:
                plate_location[4] = _x
            else:
                plate_location[4] = 0
            _y = _ai_out2[55 + 8 * (_dic_info["plateGet"] - 1)] * _height / 8192
            if _y >= _dic_info["LTy"]:
                plate_location[5] = _y
            else:
                plate_location[5] = 0
            _x = _ai_out2[56 + 8 * (_dic_info["plateGet"] - 1)] * _width / 8192
            if _x <= _dic_info["RBx"]:
                plate_location[6] = _x
            else:
                plate_location[6] = _dic_info["RBx"]
            _y = _ai_out2[57 + 8 * (_dic_info["plateGet"] - 1)] * _height / 8192
            if _y <= _dic_info["RBy"]:
                plate_location[7] = _y
            else:
                plate_location[7] = _dic_info["RBy"]

            plate1pos_confidence = _ai_out2[74 + 2 * (_dic_info["plateGet"] - 1)]
            plate2pos_confidence = _ai_out2[74 + 2 * (_dic_info["plateGet"] - 1)]
        ...
        mylogger.info("recv end with %d ms"%((100-_sleepCnt)*50))


        if _sleepCnt > 0 and msg1[1] == 0xa015:
            result_queue.put(msg1[0])

        else:
            mylogger.warning("VLPR ipc recv None in 5 seconds cnt=%d"%MSG_send_error_cnt)
