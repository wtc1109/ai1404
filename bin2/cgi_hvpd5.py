...
if None != cur:
    ...
    for _hvpd in _hvpds:
        ...
        if 0 == cur.execute("select * from reouttable where cameraID='%s' and cmosID=%d" % (_hvpd[0], _hvpd[1])):
            continue
        ...
        for i in range(3):
            _reout.append('NULL')
        if None != _reout1 and None != _reout2:
            for i in range(_spaces):
                if 1 == _aiout1[5+i] and None != _reout1[3+4*i]:
                    _reout[i]= "now: %s, ST: %s"%(_reout1[3+4*i], _reout2[4+5*i])
