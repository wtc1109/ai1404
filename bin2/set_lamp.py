    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                _ret1, err = wtclib.http_get_cgi_msg2device({"Lamp": _recv_dict['Lamp']}, ip=_recv_dict['ip'],
                                                            cgi_name="setting")
                step = _ret1
            except Exception, e:
                step = -1
                ...
                _ret_dic = {"ret": step, "id": self.__id, "error":err}
    ...
    while True:
        ...
        for i in range(Cur_qsize):
            try:
                ...
                cur.execute("update lamp_change set LampRenewTimet=%d,working=0 where cameraID='%s'"%(_timet_Sec, _dic['sn']))
                if 1 == _dic['ret']:
                    cur.execute("update lamp_change set priority_new=0 where cameraID='%s'"%(_dic['sn']))
                    cur.execute("update equipment2cameraidmem set LampStatus='%s', LampRenewTimet=%d "
                                "where equipmentID='%s'"%(_dic['Lamp'], _timet_Sec, _dic['equit']))
                    mylogger.info("%s Lamp now is %s"%(_dic['sn'], _dic['Lamp']))
                else:
                    cur.execute("update lamp_change set priority_new=%d where cameraID='%s'"%(_dic['priority'], _dic['sn']))
                    mylogger.info("%s %s Lamp thread%d set to %s fail %s"%(_dic['sn'], _dic['ip'], _dic['id'], _dic['Lamp'], _dic['error']))
                    _sleep = 0.5
                ...
                for _camera in _cameras:
                    ...
                    for i in range(THREAD_NUM):
                        if 0 != gThread_using_flag[i]:
                            ...
                            _done = 0
                            cur.execute("update lamp_change set working=1 where cameraID='%s'" % _camera[2])
                            break
