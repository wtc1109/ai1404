...
if None != cur:
    ...
    <td width="205"></td></tr>
    """
    cur.execute("select sum(CarCount) from aiouttable")
    _a2, = cur.fetchone()
    print "<tr><td></td><td>bays all:</td><td>%d</td></tr>" % (_a2)
    #print "<tr><td></td><td>all Cars:</td><td>%d</td></tr>" % (_c1 + _c2 + _c3)
    print "<tr><td></td><td>plate all:</td><td>%d</td></tr>"%(_c1 + _c2 + _c3)
    print "<tr><td></td><td>vlpr result:</td><td>%d</td></tr>" % (_d1 + _d2 + _d3)
    print "<tr><td></td><td>plate error:</td><td>%d</td></tr>"%(_err_b1 + _err_b2 + _err_b3)
    if 0 != (_c1 + _c2 + _c3):
        _percent = 1.0 * (_d1 + _d2 + _d3 - (_err_b1 + _err_b2 + _err_b3)) / (_c1 + _c2 + _c3)
        print "<tr><td></td><td>OK persent:</td><td>%f</td></tr>" % (_percent)
        print "<tr><td></td><td>Error persent:</td><td>%f</td></tr>"%(1.0- _percent)
