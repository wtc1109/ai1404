...
#                bitmap_strs.append(cur.fetchmany(cur.execute(strInfo)))
            ...
            mylogger.debug("select output from " ,LampCtrlSN_name,b1, space_bitmap)
            if 0 != b1:
...
if __name__ == '__main__':
    global mylogger
    mylogger = create_logging("checksql.log")
    ...
    while True:
        ...
        if cnt < 0:
            cnt = 2000
            mylogger.info("checksql is running")
        if 0 == cnt%200:
            mylogger.debug("checksql is running")
