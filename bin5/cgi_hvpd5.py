...
if None != cur:
    ...
    for _hvpd in _hvpds:
        ...
        for i in range(_bays):
            ...
            _now = ''
            if 1 == _state:
                if 0 != cur.execute("select PlateNumber from reouttable_bay where cameraID='%s' and cmosID=%d and bay=%d" % (_hvpd[0], _hvpd[1],i+1)):
                    _now, = cur.fetchone()
                    if None == _now:
                        _now = ''
        ...
        for i in range(_bays):
            _reout.append('NULL')
            if '' == _reout1[i] and '' == _reout2[i]:
                print "<p>NULL</p>"
            else:
                print "<p>now: %s, ST: %s</p>"%(_reout1[i], _reout2[i])
