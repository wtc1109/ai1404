...
def Udp_broadcast(addr, port):
    ...
    while True:
        ...
        msg = {"AISERVERIP": addr, "AIport": RECEIVE_PORT, "timenow": time_str, "timstr": time_strF, \
               "sec": Encrypt(time_strF), "AIsn": _serverID, "AIHW": "AIBC", "VERSION": "2.00", "Online": "3"}
        ...
        mylogger.info("udp broadcast:" + js_msg + " @ " + addr)
        time.sleep(10)
...
if __name__ == '__main__':
    ...
    mylogger = wtclib.create_logging("../log/udplog/udpbroad1.log")
    mylogger.info("start running")
    addr = get_ip_addr("eth1")
    if None == addr:
        mylogger.error("eth1 addr is None")
        os._exit(0)
    cnt = 10
    while True:
