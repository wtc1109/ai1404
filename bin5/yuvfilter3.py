...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            _sql_str = "select cameraID from spacestatustable where YUVRenewTime<%d and YUVRenewTime<>0 and CarportStatus=1"%(int(_timeSec)-_haveCar_delay)

            a1 = cur.execute(_sql_str)
            mylogger.info("fetchmany(%d) need yuv files " % a1 + _sql_str)
            _cameras = set(cur.fetchmany(a1))
        ...
        for _camera in _cameras:
            _sql_str = "update spacestatustable set YUVRenewTime=0 where cameraID='%s'"%_camera[0]
            cur.execute(_sql_str)
            msg = json.dumps({"sn": _camera[0], "CMOS": 0, "from":"yuvfilter"})
