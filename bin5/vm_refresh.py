        ...
        try:
            ...
            for i in range(13):
                ...
                if 0 == _bays['%d%d' % (1809435000 + _offset, _bay)]:
                    _sql_str = "update spacestatustable set CarportStatus=1 where cameraID='%d' and bay=%d"%(1809435000 + _offset,_bay)
                    _bays['%d%d' % (1809435000 + _offset, _bay)] = 1
                else:
                    _sql_str = "update spacestatustable set CarportStatus=0 where cameraID='%d' and bay=%d" % (1809435000 + _offset, _bay)
