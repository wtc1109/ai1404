    ...
    for i in range(int(_ScreenConfig[4])):
        ...
        try:
            ...
            if "\"%c\"" == _ScreenRegionConf[8]:
                ...
                _sql_str = "select count(*) from SpaceStatusTable sst," \
                           "(select cameraID,bay from Space2LedTable where RegionSn=%d) as t " \
                           "where sst.cameraID = t.cameraID and sst.bay = t.bay and sst.CarportStatus=0;" % _ScreenConfig[5 + i]
                    ...
                    if 0 == _ret:
                        ...
                        if 0 != len(_data_recv):
                            ...
                            if "LEDREGIONSDISPLAY" == _recv[0]:
                                if len(_recv) > 2:
                                    _sql_str = "update ScreenConnectTable set returnval='%s %s'"%(_recv[1],_recv[2])
                                else:
                        ...
                        _sql_str = "update ScreenConnectTable set returnval='%s'"%msg
                        _cur.execute(_sql_str)
                except Exception, e:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        mylog.debug("process leddisplayer %d leds in %f second"%(leds, timeused))
        if timeused < 3:
